...
from rdkit.Chem import AllChem, rdMolTransforms, ChiralType, TorsionFingerprints
...
def mol_to_data_chiro(mol):
    ...
    data = Data(x = torch.as_tensor(node_features), edge_index = torch.as_tensor(edge_index, dtype=torch.long), edge_attr = torch.as_tensor(edge_features), pos = pos)
